...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.event === "payment_completed") {
      ...
      await fetch(`${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/email/send-reminder`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    return NextResponse.json({ success: true, message: "Email webhook processed" })
  } catch (error) {
